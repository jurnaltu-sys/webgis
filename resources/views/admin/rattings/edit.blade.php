...
@section('content')
    ...
    <div class="card border-primary">
        ...
        <div class="card-body">
            <form action="{{ route('rattings.update', $ratting) }}" method="POST">
                @csrf
                @method('PUT')
                <!-- User di baris pertama -->
                <div class="form-group">
                    <label for="user_id">User</label>
                    <select name="user_id" id="user_id" class="form-control select2 @error('user_id') is-invalid @enderror" required>
                        <option value="">Pilih user</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ (string) old('user_id', $ratting->user_id) === (string) $user->id ? 'selected' : '' }}>
                ...
                <!-- Baris berikutnya mirip rating_wisatawan/edit.blade.php -->
                <div class="form-row">
                ...
                <div class="d-flex">
                    <a href="{{ route('rattings.index') }}" class="btn btn-secondary mr-2">Batal</a>
                    <button type="submit" class="btn btn-primary">Simpan</button>
                </div>
                <!-- Modal preview gambar wisata -->
